...
int main(void)
{
    ...
    if (!url || !user || !pass) {
        fprintf(stderr,
                "integration: set TTRSS_URL, TTRSS_USER and TTRSS_PASS\n");
        return 2;
    }
    ...
    Feed *feeds = NULL;
    size_t nfeeds = 0;
    REQUIRE(api_get_feeds(api, CATEGORY_ALL, false, false, &feeds, &nfeeds) ==
                0,
            "getFeeds");
    ...
    Headline *heads = NULL;
    size_t nheads = 0;
    REQUIRE(api_get_headlines(api, feed_id, false, 20, 0, VIEW_ALL_ARTICLES,
                              NULL, false, true, &heads, &nheads) == 0,
            "getHeadlines");
    ...
    if (nheads > 0) {
        ...
        REQUIRE(api_update_article(api, &id, 1, UA_MODE_TOGGLE,
                                   UA_FIELD_MARKED, NULL) == 0,
                "updateArticle toggle marked");
        REQUIRE(api_update_article(api, &id, 1, marked ? UA_MODE_TRUE
                                                       : UA_MODE_FALSE,
                                   UA_FIELD_MARKED, NULL) == 0,
