...
{
    render_wrap("", app->art_wrap_width, COLOR_PAIR(CP_DEFAULT),
...
void article_rebuild(App *app)
{
    ...
    render_wrap(a->title ? a->title : "(untitled)", width,
                COLOR_PAIR(CP_TITLE) | A_BOLD, &app->art_lines,
                &app->art_nlines, &cap);
    add_blank(app, &cap);
    ...
    if (a->note && *a->note) {
        add_blank(app, &cap);
        render_wrap("Note:", width, COLOR_PAIR(CP_NOTE) | A_BOLD,
                    &app->art_lines, &app->art_nlines, &cap);
    ...
    if (a->nattachments > 0) {
        ...
        for (size_t i = 0; i < a->nattachments; i++) {
            render_wrap(a->attachments[i].url ? a->attachments[i].url : "",
                        width, COLOR_PAIR(CP_DEFAULT), &app->art_lines,
                        &app->art_nlines, &cap);
        }
    }
}
...
void article_draw(App *app)
{
    ...
    if (!app->article) {
        render_text(w, 1, 2, app->art_cols - 3,
                    "(select a headline to read)", COLOR_PAIR(CP_READ));
        wnoutrefresh(w);
        return;
    }
