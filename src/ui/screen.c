...
void ui_layout(App *app) {
  ...
  if (app->head_rows > 0) {
    app->head_limit = app->head_rows;
  } /* re-clamp page size on resize */
...
static void next_unread(App *app) {
  ...
  for (size_t k = 0; k < app->nheads; k++) {
    ...
    if (app->heads[i].unread) {
      ...
      if (app->head_rows > 0 &&
          app->head_sel >= app->head_top + app->head_rows) {
...
int ui_run(App *app) {
  ...
  if (app->cfg->theme && *app->cfg->theme &&
      !render_theme_valid(app->cfg->theme)) {
    ui_status(app, true, "Unknown theme \"%s\"; using default",
              app->cfg->theme);
  }
