...
static const char *default_browser(void) {
...
Config *config_new(void) {
  ...
  cfg->insecure = false;
  cfg->debug = false;
  cfg->browser = xstrdup(default_browser());
  cfg->theme = xstrdup("dark");
  const char *home = getenv("HOME");
...
int config_load_file(Config *cfg, const char *path) {
  ...
  if (!root) {
    snprintf(cfg->error, sizeof cfg->error, "cannot read config %s: %s",
             expanded, err.text);
    free(expanded);
    return -1;
  }
  ...
  if ((s = cfg_json_str(root, "browser"))) {
    set_str(&cfg->browser, s);
  }
  if ((s = cfg_json_str(root, "theme"))) {
    set_str(&cfg->theme, s);
  }
...
int config_validate(Config *cfg) {
  ...
  if (!cfg->browser || !*cfg->browser) {
    set_str(&cfg->browser, default_browser());
  }

  if (!cfg->theme || !*cfg->theme) {
    set_str(&cfg->theme, "dark");
  }
